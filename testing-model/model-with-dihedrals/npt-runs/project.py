...
def run(job):
    ...
    with job:
        ...
        print("NVT step finished, running NPT...")
        sim.run_NPT(
...
def run_nvt(job):
    ...
    with job:
        ...
        gsd_path = job.fn(f"nvt-trajectory{job.doc.nvt_runs}.gsd")
        log_path = job.fn(f"nvt-log{job.doc.nvt_runs}.txt")
        ref_values = get_ref_values(job)
        sim = Simulation(
            initial_state=job.fn("npt-restart.gsd"),
            forcefield=hoomd_ff,
        ...
        target_box = get_target_box_mass_density(
                mass=sim.mass.to("g"),
                density=job.doc.density_avg * Unit("g/cm**3")
        )
        job.doc.nvt_target_box = target_box.value
        shrink_kT_ramp = sim.temperature_ramp(
                n_steps=1e7,
                kT_start=job.sp.kT,
                kT_final=job.sp.kT
        ...
        sim.save_restart_gsd(job.fn("nvt-restart.gsd"))
        job.doc.runs += 1
        print("Simulation finished.")
